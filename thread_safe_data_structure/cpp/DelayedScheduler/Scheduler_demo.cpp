...
int main() {
  ...
  PACKAGE_NAME::Scheduler s(max_n_threads);
  ...
  s.at("2018-05-06 12:32:45", []() { std::cout << "at a specific time." << std::endl; });

  s.cron("5 0 * * *", []() { std::cout << "every day 5 minutes after midnight" << std::endl; });

  // destructor of PACKAGE_NAME::Scheduler will cancel all schedules but finish any tasks currently running
